              ...
              {stats.monthlyChartData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={stats.monthlyChartData}>
                    ...
                    <Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 12px 36px rgb(0 0 0 / 0.14)' }} />
                    <Bar dataKey="count" radius={[8, 8, 8, 8]}>
                      {stats.monthlyChartData.map((entry, index) => <Cell key={`${entry.name}-${index}`} fill={extractedColors?.primary || '#0071e3'} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
          ...
          <div className="apple-surface apple-surface-interactive rounded-[2.5rem] p-7 sm:p-10">
            ...
            <div className="relative mt-6 h-72 w-full">
              {publishedPosts.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={pieData} cx="50%" cy="50%" innerRadius={68} outerRadius={92} paddingAngle={4} dataKey="value" cornerRadius={8}>
                      {pieData.map((entry, index) => <PieCell key={`${entry.name}-${index}`} fill={entry.color} />)}
                    </Pie>
