...
export default function Home() {
  ...
  const compositionData = [
    { name: '鏃ヨ', value: stats.diary, color: extractedColors?.secondary || '#34c759' },
    { name: '瀛︿範', value: stats.learning, color: extractedColors?.primary || '#0071e3' },
  ];
    ...
    <div className="page-enter space-y-24 md:space-y-32">
      <section className="flex min-h-[calc(100svh-8rem)] max-w-6xl flex-col justify-end pb-10 text-white md:min-h-[calc(100svh-6rem)] md:pb-16">
        <p className="hero-text-shadow mb-6 text-sm font-semibold uppercase tracking-[0.2em] text-white/80">
          Personal archive 路 {photos.length} moments
        </p>
      ...
      <section id="today" className="scroll-mt-10 space-y-10">
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div>
            ...
            <h2 className="section-title max-w-3xl">浠婂ぉ锛屽厛鐪嬬湅鍙戠敓浜嗕粈涔堛€?/h2>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <Link to="/news" className="apple-surface apple-surface-interactive group rounded-[2.5rem] p-7 sm:p-10">
            ...
            <h3 className="mt-3 text-3xl font-semibold tracking-[-0.045em] text-zinc-950 dark:text-white">鏂伴椈璧勮</h3>
            <p className="mt-4 max-w-md leading-relaxed text-zinc-500 dark:text-zinc-400">澶氭簮瀹樻柟 RSS锛岀湡瀹炴潵婧愪笌閾炬帴閮藉湪鐙珛鏂伴椈椤典腑銆?/p>
            <span className="mt-9 inline-flex items-center gap-1 text-sm font-semibold text-zinc-600 transition group-hover:text-[rgb(var(--theme-primary))] dark:text-zinc-300">
              鏌ョ湅鏂伴椈 <ArrowUpRight className="h-4 w-4" />
            </span>
          </Link>
          ...
          <Link to="/weight" className="apple-surface apple-surface-interactive group rounded-[2.5rem] p-7 sm:p-10">
            ...
            <h3 className="mt-3 text-3xl font-semibold tracking-[-0.045em] text-zinc-950 dark:text-white">浣撻噸璁板綍鍣?/h3>
            <p className="mt-4 max-w-md leading-relaxed text-zinc-500 dark:text-zinc-400">鎶婃暟鎹笌瓒嬪娍鐣欏湪鐙珛椤甸潰锛屼笓娉ㄨ褰曟瘡涓€娆″彉鍖栥€?/p>
            <span className="mt-9 inline-flex items-center gap-1 text-sm font-semibold text-zinc-600 transition group-hover:text-[rgb(var(--theme-primary))] dark:text-zinc-300">
              鎵撳紑璁板綍鍣?<ArrowUpRight className="h-4 w-4" />
            </span>
          </Link>
        </div>
      </section>
      ...
      <section className="space-y-10">
        <div>
          ...
          <h2 className="section-title">璁╄褰曞舰鎴愯嚜宸辩殑鑺傚銆?/h2>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="apple-surface apple-surface-interactive rounded-[2.5rem] p-7 sm:p-10 lg:col-span-2">
            <div className="mb-10 flex items-end justify-between">
              <div>
                <p className="text-sm font-semibold text-zinc-500">杩戝崐骞村彂甯冭秼鍔?/p>
                <p className="mt-2 text-4xl font-semibold tracking-[-0.045em]">
                  {publishedPosts.length}<span className="ml-2 text-base font-medium text-zinc-400">绡囪褰?/span>
                </p>
              </div>
              <span className="rounded-full bg-[rgb(var(--theme-primary)/0.10)] px-3 py-2 text-xs font-semibold text-[rgb(var(--theme-primary))]">鐪熷疄鍙戝竷鏁版嵁</span>
            </div>
            <div className="h-72 w-full overflow-hidden rounded-[1.75rem] border border-black/[0.05] bg-gradient-to-br from-white/80 to-[rgb(var(--theme-primary)/0.08)] p-3 dark:border-white/10 dark:from-white/[0.06] dark:to-[rgb(var(--theme-primary)/0.14)]">
              ...
              ) : stats.monthlyChartData.length === 1 ? <div className="flex h-full flex-col items-center justify-center text-center"><span className="mb-4 h-4 w-4 rounded-full bg-[rgb(var(--theme-primary))] ring-8 ring-[rgb(var(--theme-primary)/0.12)]" /><p className="text-sm font-semibold text-zinc-700 dark:text-zinc-200">{stats.monthlyChartData[0].name} 路 {stats.monthlyChartData[0].count} 绡?/p><p className="mt-2 text-sm text-zinc-500">鍐嶇Н绱竴涓湀浠斤紝瓒嬪娍浼氳嚜鐒跺嚭鐜般€?/p></div> : <div className="flex h-full items-center justify-center text-sm text-zinc-400">鍙戝竷绗竴绡囪褰曞悗锛岃繖閲屼細寮€濮嬪舰鎴愯秼鍔裤€?/div>}
            </div>
          </div>

          <div className="apple-surface apple-surface-interactive rounded-[2.5rem] p-7 sm:p-10">
            <p className="text-sm font-semibold text-zinc-500">鍐呭鍗犳瘮</p>
      ...
      <section className="space-y-10">
        <div className="flex items-end justify-between gap-6">
          <div>
            ...
            <h2 className="section-title">鏈€杩戝啓涓嬬殑鏁呬簨銆?/h2>
          </div>
          <Link to="/posts" className="hidden items-center gap-1 text-sm font-semibold text-zinc-600 hover:text-zinc-950 sm:flex dark:text-zinc-300 dark:hover:text-white">
            鏌ョ湅鍏ㄩ儴 <ArrowUpRight className="h-4 w-4" />
          </Link>
        </div>
        ...
        {recentPosts.length > 0 ? (
          ...
          <div className="apple-surface rounded-[2.5rem] p-16 text-center">
            <p className="mb-4 text-zinc-500">杩樻病鏈変换浣曡褰曪紝蹇幓鍙戝竷绗竴绡囨枃绔犲惂锛?/p>
          </div>
        )}
      </section>
    </div>
